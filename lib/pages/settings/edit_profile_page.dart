...
import '../../theme.dart';
import '../../services/siswa_profile_service.dart';
...
class _EditProfilePageState extends State<EditProfilePage>
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      final data = await SiswaProfileService.getProfile();
    ...
    } on SiswaProfileServiceException catch (e) {
      if (mounted) {
        setState(() {
  ...
  Future<void> _pickAvatar() async {
    ...
    try {
      ...
      final newUrl = await SiswaProfileService.uploadAvatar(file);
      if (mounted) {
        setState(() {
          _avatarUrl = newUrl;
      ...
      if (mounted) {
        ...
        _showSnackBar(
          e is SiswaProfileServiceException ? e.message : 'Gagal upload foto',
          isSuccess: false,
        );
      }
    }
  ...
  Future<void> _selectDiceBearAvatar(String url) async {
    ...
    try {
      final savedUrl = await SiswaProfileService.setAvatarUrl(url);
      if (mounted) {
        setState(() {
          _avatarUrl = savedUrl;
          _isUploadingAvatar = false;
      ...
      if (mounted) {
        setState(() => _isUploadingAvatar = false);
        _showSnackBar(
          e is SiswaProfileServiceException
              ? e.message
  ...
  Future<void> _saveProfile() async {
    ...
    if (email.isNotEmpty && !email.contains('@')) {
      _showSnackBar('Format email tidak valid', isSuccess: false);
      return;
    }
    ...
    try {
      await SiswaProfileService.updateProfile(
        name: name,
        email: email,
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          // Class info section
          if (_profileData != null && _profileData!.className != '-')
            _buildClassSection(),
