<?php
// /server/api/public_speaking.php
// Endpoint CRUD catatan Public Speaking siswa
...
// ════════════════════════════════════════
// GET: Ambil semua catatan public speaking
// ════════════════════════════════════════
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        $stmt = $pdo->prepare(
            'SELECT id, title, speaking_date, materi, mentor, note, points_earned, created_at
             FROM public_speaking_logs
             WHERE student_id = :sid
             ORDER BY speaking_date DESC, created_at DESC'
        );
...
// Body JSON: { id?, date, judul, materi, mentor, note }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($noteId) {
            // ── Update ──
            // Pastikan catatan milik siswa ini
            $check = $pdo->prepare('SELECT id FROM public_speaking_logs WHERE id = :id AND student_id = :sid');
            ...
            $stmt = $pdo->prepare(
                'UPDATE public_speaking_logs
                 SET title = :title, speaking_date = :d, materi = :materi, mentor = :mentor, note = :note
            ...
            $points = 2; // Poin per catatan public speaking

            $stmt = $pdo->prepare(
                'INSERT INTO public_speaking_logs (student_id, title, speaking_date, materi, mentor, note, points_earned)
            ...
            // Tambah poin ke students — recalculate dari semua sumber
            $grandTotal = 0;
            ...
            // Public speaking (including the one just inserted)
        ...
        if (!$noteId) {
            try {
                ...
                if ($roleRow && $roleRow['role'] === 'siswa') {
                    ...
                    if (!empty($parentIds)) {
                        $taskLabel = $judul ?: 'Public Speaking';
                        $notifTitle = 'Ananda Baru Saja Mengerjakan Tugas';
                        $notifBody  = "Ananda {$studentName} baru saja mengerjakan tugas Public Speaking ({$taskLabel}), klik untuk melihat!";

                        sendFcmToUsers($pdo, $parentIds, $notifTitle, $notifBody, [
                            'type'       => 'task_submitted',
                            'task_type'  => 'public_speaking',
                            'task_name'  => $taskLabel,
                ...
                error_log('[FCM Task] Public Speaking FCM error: ' . $fcmEx->getMessage());
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    try {
        ...
        $check = $pdo->prepare('SELECT id, points_earned FROM public_speaking_logs WHERE id = :id AND student_id = :sid');
        $check->execute(['id' => $noteId, 'sid' => $studentId]);
        $row = $check->fetch();
        ...
        // Hapus catatan
        $stmt = $pdo->prepare('DELETE FROM public_speaking_logs WHERE id = :id AND student_id = :sid');
        $stmt->execute(['id' => $noteId, 'sid' => $studentId]);

        // Kurangi poin — recalculate dari semua sumber
        $grandTotal = 0;
        ...
        // Public speaking (after deletion)
        ...
        if ($earnedPoints > 0) {
            ...
            $pdo->prepare(
                ...
                'sid'   => $studentId,
                'src'   => 'public_speaking_delete',
                'srcid' => $noteId,
                'pts'   => -$earnedPoints,
                'desc'  => 'Hapus catatan Public Speaking',
            ]);
        }
